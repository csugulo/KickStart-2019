...
int main(){
    ...
    for (int t = 1; t <= T; t++){
        ...
        vector<vector<vector<int>>> dp(siz, vector<vector<int>>(K+1, vector<int>(2, 0x3f3f3f3f) ));
        for (int i=0; i<siz; i++){
            dp[i][0][0] = 0;
            dp[i][0][1] = 0;
        }
        ...
        for (int i=1; i<siz; i++){
            ...
            for (int j=1; j<=K; j++){
                ...
                for (int d=0; d<color_distances[clr].size(); d++){
                    if (j-d-1 < 0) break;
                    dp[i][j][0] = min(dp[i][j][0], dp[i-1][j-d-1][0]+2*color_distances[clr][d] );
                    dp[i][j][1] = min(dp[i][j][1], dp[i-1][j-d-1][1]+2*color_distances[clr][d] );
                }
            ...
            for (int j=1; j<=K; j++){
                ...
                for (int d=0; d<color_distances[clr].size(); d++){
                    if (j-d-1 < 0) break;
                    dp[i][j][1] = min(dp[i][j][1], dp[i-1][j-d-1][0] + color_distances[clr][d] );
                }
            }

            
        }
        res = min(dp[siz-1][K][0], dp[siz-1][K][1]);
